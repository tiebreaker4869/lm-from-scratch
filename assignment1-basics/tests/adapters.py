...
from cs336_basics.training_utils import cross_entropy, learning_rate_schedule, gradient_clipping, get_next_batch, load_checkpoint, save_checkpoint
...
def run_multihead_self_attention(
    ...
    """
    multihead_sa = MultiHeadSelfAttention(d_model, num_heads)
    multihead_sa.w_qkv.w.data = torch.concat([q_proj_weight, k_proj_weight, v_proj_weight], dim=0)
    multihead_sa.w_out.w.data = o_proj_weight
    out = multihead_sa(in_features)
    return out
...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    multihead_sa = MultiHeadSelfAttention(d_model, num_heads, theta, max_seq_len)
    multihead_sa.w_qkv.w.data = torch.concat([q_proj_weight, k_proj_weight, v_proj_weight], dim=0)
    multihead_sa.w_out.w.data = o_proj_weight
    out = multihead_sa(in_features, token_positions)
    return out
